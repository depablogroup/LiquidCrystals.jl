...
#---------------------------Homeotropic Boundary Conditions Structs---------------------------------------------------------

"""
	HomeotropicAxisBC{A <: Axis} <: AxisBC

A struct that sets up the homeotropic boundary conditions for a 2D simulation box.

	HomeotropicAxis{XAxis}(S::Real, W_l::Real, W_h::Real)
	HomeotropicAxis{YAxis}(S::Real, W_l::Real, W_h::Real)

The axes needs to be accessed as LiquidCrystals.XAxis and LiquidCrystals.YAxis 
"""
struct HomeotropicAxisBC{A <: Axis} <: AxisBC
	Q0_l::SVector  	# The anchoring Q tensor (Svector): lower boundary
	W_l::Real 		# The anchoring strength: lower boundary
	Q0_h::SVector  	# The anchoring Q tensor (Svector): higher boundary 
	W_h::Real 		# The anchoring strength: higher boundary

	function HomeotropicAxisBC{A}(S::Real, W_l::Real, W_h::Real) where {A <: XAxis}
		Q0_l = S * SVector(-1.0, 0.0, 1.0) * 0.5
		Q0_h = S * SVector(-1.0, 0.0, 1.0) * 0.5
		return new{A}(Q0_l, W_l, Q0_h, W_h)
	end

	function HomeotropicAxisBC{A}(S::Real, W_l::Real, W_h::Real) where {A <: YAxis}
		Q0_l = S * SVector(1.0, 0.0, -1.0) * 0.5
		Q0_h = S * SVector(1.0, 0.0, -1.0) * 0.5
		return new{A}(Q0_l, W_l, Q0_h, W_h)
	end
...
"""
	PlanarAxisBC{A <: Axis} <: AxisBC

A struct that sets up the planar boundary conditions for a 2D simulation box.

	PlanarAxis{XAxis}(S::Real, W_l::Real, W_h::Real)
	PlanarAxis{YAxis}(S::Real, W_l::Real, W_h::Real)

The axes needs to be accessed as LiquidCrystals.XAxis and LiquidCrystals.YAxis 
"""
struct PlanarAxisBC{A <: Axis} <: AxisBC
	Q0_l::SVector  	# The anchoring Q tensor (Svector): lower boundary
	W_l::Real 		# The anchoring strength: lower boundary
	Q0_h::SVector  	# The anchoring Q tensor (Svector): higher boundary 
	W_h::Real 		# The anchoring strength: higher boundary

	function PlanarAxisBC{A}(S::Real, W_l::Real, W_h::Real) where {A <: XAxis}
		Q0_l = S * SVector(1.0, 0.0, -1.0) * 0.5
		Q0_h = S * SVector(1.0, 0.0, -1.0) * 0.5
		return new{A}(Q0_l, W_l, Q0_h, W_h)
	end

	function PlanarAxisBC{A}(S::Real, W_l::Real, W_h::Real) where {A <: YAxis}
		Q0_l = S * SVector(-1.0, 0.0, 1.0) * 0.5
		Q0_h = S * SVector(-1.0, 0.0, 1.0) * 0.5
		return new{A}(Q0_l, W_l, Q0_h, W_h)
	end
...
function initialize(op, bc::HomeotropicAxisBC{A}, extended) where {A <: Axis}
...
#-------------------------------HomeotropicAxis Boundary Ranges-------------------------------------

function boundary_ranges(sz::NTuple{2}, ::HomeotropicAxisBC{YAxis})
	m, n = sz

	#extract two as it is the extended matrix size
	lo = CartesianIndices((1:m-2, 1:1))
	hi = CartesianIndices((1:m-2, n-2:n-2))

	return lo, hi
end

function boundary_ranges(sz::NTuple{2}, ::HomeotropicAxisBC{XAxis})
	m, n = sz
...
#-------------------------------PlanarAxis Boundary Ranges-------------------------------------------

function boundary_ranges(sz::NTuple{2}, ::PlanarAxisBC{YAxis})
	m, n = sz
...
function boundary_ranges(sz::NTuple{2}, ::PlanarAxisBC{XAxis})
	m, n = sz
...
#----------------------------------Homeotropic and Planar BCs----------------------------------------------------

function apply_BCs(op, u, bc::LiquidCrystals.HomeotropicAxisBC{A}, extended) where {A <: LiquidCrystals.Axis}
	...
	CI_lg = CartesianIndices(lo_g)  #1st axis and low-value of axis
	extended[CI_lg] .= bc.W_l[1] * (u[CI_lb] .- Ref(bc.Q0_l))
	
	CI_hg = CartesianIndices(hi_g)  #1st axis and high-value of axis
    extended[CI_hg] .= bc.W_h[1] * (u[CI_hb] .- Ref(bc.Q0_h))
	
    return extended
end

function apply_BCs(op, u, bc::LiquidCrystals.PlanarAxisBC{A}, extended) where {A <: LiquidCrystals.Axis}
