...
begin
	const Nx = 70
	const Ny = 20  # Dimensions of the space discretization
...
begin
	...
	#ϕ₀ =fill(c₀, (Nx,Ny)) #c₀ .+ 2δc .* rand(Nx, Ny) .- δc  #
...
begin	
	...
	const Mc =0#1e-24  # Gamma/rho

	const c₀ = 0.9  # Mean value of the concentration of one of the species
	const δc = 0.0001  # Size of the fluctuations in the initial concentration
...
# ╔═╡ ce67a1ac-9b0a-4bbc-8e84-de7663b367b6
	params = (∇, Δ, bc_box, E_∇ϕ, P_∇ϕ, E_Δϕ, P_Δϕ, E_Δμ_ϕ, P_Δμ_ϕ, bc_box, E_∇Q, P_∇Q, E_Q, P_Q, E_ϕQ, P_ϕQ, A, U)
...
#=╠═╡
llll=reinterpret(reshape, ST, QQQQq)
  ╠═╡ =#
...
#=╠═╡
LiquidCrystals.s_and_directors(QQQQq)
  ╠═╡ =#
...
function paramovie2(sol, space, path = "q_data")
	...
	#for i in 1:nₜ
		...
		Ss, n̂s = LiquidCrystals.s_and_directors(sol)
		writevtk(name, space, Ss, n̂s)
	#end
end
