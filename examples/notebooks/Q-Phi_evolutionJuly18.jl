...
begin
	const Nx = 200
	const Ny = 200  # Dimensions of the space discretization
...
# ╔═╡ 3785fade-af72-4a4c-bc6f-3550fb1cfeea


# ╔═╡ 5b32f8d1-c668-4b36-b1a6-0fc2e26bedd2


# ╔═╡ cf7b6bfc-6dfc-4919-8bf4-8ebb8a3cba0d
begin
	const Y = ([i for i in 1:Ny for j in 1:Nx])
	const X = ([j for i in 1:Ny for j in 1:Nx])
...
# ╔═╡ 540296ae-dcec-43e3-91fc-3448a005ed69


# ╔═╡ c76608e0-da88-4c4f-b04a-4d40ec3014bd


# ╔═╡ 395777a1-e84a-4b08-98b5-15381c1e2067


# ╔═╡ 9160dc00-0da5-4efe-9c88-3fddaf1fe8e6
...
begin	
	...
	const K = 2.0*dx/T#3e-9
	const Kₑ= 1.0#1e-12
	# const Da = 1.0e-04 * exp(-3e5 / (R * T))
	# const Db = 2.0e-05 * exp(-3e5 / (R * T)) 
	# const Mc = (Da / (R * T)) * (c0 + (Db/Da) * (1.0 - c0)) * c0 * (1.0 - c0) 
	const Mc =1.5*1e-7#1e-25  # Gamma/rho
	const kappa= 2.0

	const c₀ = 0.95  # Mean value of the concentration of one of the species
...
nx,ny=dir_curved_droplets(Nx,Ny, X, Y, 8, 12, 12)

# ╔═╡ fe18ae4d-c8d7-4752-8c0d-2dd871beb279
begin
		U = 6  # Initial value as U = B/A = Phi if B=Phi^2 and A=Phi
...
phi0, S0= phi_S_2droplets(Nx,Ny,X, Y, S,S,0.05,c₀, 12, 12, 8)
...
begin
	...
	#ϕ₀ = rand(Nx, Ny) # c₀ .+ 2δc .* rand(Nx, Ny) .- δc #fill(c₀, (Nx,Ny)) #c₀ .+ 2δc .* rand(Nx, Ny) .- δc  #
...
begin
	...
	dϕ = volterra_ϕ(ϕQ0, A, U, 1) +(Δϕ)  

	ΔQ = @view recover_Q(P_ϕQ)[2:end-1, 2:end-1]

	dQ =  volterra_Q(ϕQ0, A, U, 1) +ΔQ 
...
# ╔═╡ ce67a1ac-9b0a-4bbc-8e84-de7663b367b6
	params = (∇, Δ, bc_box, E_∇ϕ, P_∇ϕ, E_Δϕ, P_Δϕ, E_Δμ_ϕ, P_Δμ_ϕ, E_∇Q, P_∇Q,  E_ϕQ, P_ϕQ, A, U, div, E_divϕQ,	P_divϕQ)
...
function ϕ_Q(du, u, p, t)
	...
	∇, Δ, bc_box, E_∇ϕ, P_∇ϕ, E_Δϕ, P_Δϕ, E_Δμ_ϕ, P_Δμ_ϕ, E_∇Q, P_∇Q, E_ΔϕQ, P_ΔϕQ, A, U, div, E_divϕQ, P_divϕQ= p
...
# ╔═╡ 5b2f4f25-24dc-40d5-ae88-3801c5228d6b


# ╔═╡ 5d80823f-a628-4ead-bab7-6c4f8f8c4238


# ╔═╡ b8df7bf8-668e-44fd-a3ce-ff8ea09c81ef


# ╔═╡ 8ca762cb-866c-4872-b832-8bb43dee85a7


# ╔═╡ 4c8fc686-e495-4449-b311-1510f8ce4fc2


# ╔═╡ 332bd74f-4f59-4c22-a387-39a4ba19f520


# ╔═╡ 51ea08c9-776a-4777-9a17-d3abe665abc1


# ╔═╡ 4f36cef1-d359-44ab-95a9-251327d3b3d9
...
# ╔═╡ dc18dfba-4f87-44e8-a4d8-fc29b641b9e5
