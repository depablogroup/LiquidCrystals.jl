...
# ╔═╡ 3384098a-aeef-11ec-0d31-ebadfa76989e
begin
...
# Alias Pkg so it doesn't interfere with Pluto's package manager
import Pkg as JLPkg
...
# ╔═╡ b4dbebe9-ae27-4f86-8b8a-f8b7bcb8420c
md"""
# Cahn–Hilliard Equation
(*with logarithmic Flory–Huggins energy potential*)
\
"""

# ╔═╡ a1dfb283-753c-4914-b42c-22679001cbac
md"""
### Preamble

Let's load some dependencies first
"""

# ╔═╡ fc71e78e-fd8a-45c7-8eaf-2832feaa4fa0
md"""
### System definition

Let's first define all constants and parameters of our binary mixture:
"""

# ╔═╡ 2712dd6a-0e34-4ce9-9140-2fd1f2b52284
md"""
- Gas constant in J mol⁻¹ K⁻¹
"""

# ╔═╡ c51d24a7-085a-4c8e-93d3-dae613791a3c
const R = 8.31446261815324

# ╔═╡ 2ff33856-dfa1-401f-9663-24de0d10a5b1
md"""
- Temperature in K
"""

# ╔═╡ 07b99e75-152a-45b9-8c93-24456e4af9bb
const T = 673 

# ╔═╡ 1b0c1215-7d4e-4865-97fe-234f583c8d6e
md"""
- Flory-Huggins interaction parameter
"""

# ╔═╡ 474e0797-777b-4a29-91a1-713d1d3bb330
const χ = 20.71 * T

# ╔═╡ a5f5d0d7-16bf-4648-8fc1-1d2086b28c04
const K = 3e-14

# ╔═╡ 6d3d4db6-83be-479e-926b-6cc2116dec2f
const Da = 1.0e-04 * exp(-3e5 / (R * T)) 

# ╔═╡ 659d475b-c370-4847-8ce5-547fa16f5cee
const Db = 2.0e-05 * exp(-3e5 / (R * T)) 

# ╔═╡ 931ea644-e583-42a3-8534-4238d03b0558
md"""
- Mean value of the concentration of one of the species
"""

# ╔═╡ e706ef33-822e-4fdc-904c-4c56b3d82f66
const c₀ = 0.5

# ╔═╡ a7f03f87-a6c6-41c5-b9df-ac0c9415628f
md"""
- Size of the fluctuations in concentration in the initial state
"""

# ╔═╡ cc827e4c-00fd-44e7-a5f1-ae626cafdc44
const δc = 0.001

# ╔═╡ c077aa6e-1ac0-4612-8095-109523583aee
const Mc = 1e-23 # (Da / (R * T)) * (c0 + (Db/Da) * (1.0 - c0)) * c0 * (1.0 - c0) 

# ╔═╡ f26e33c7-a9ba-4188-890e-5477564773e7
md"""
- Dimensions of the space discretization
"""

# ╔═╡ 16a27bb5-7da4-4a3f-a9f7-6de029db42f6
const Nx = Ny = 50

# ╔═╡ 8c8bbdbe-cca5-4087-87c9-72e2a0f83767
md"""
We now randomly initialize the concentration field (*run the next cell again if the solver throws a DomainError*).
"""

# ╔═╡ f86699c3-f3a7-471c-bb93-4a548140f99b
ϕ₀ = c₀ .+ 2δc .* rand(Nx, Ny) .- δc;

# ╔═╡ 67ce9b8c-cd01-4eca-a41e-d49effad6fe1
md"""
### Numerical solution
...
# ╔═╡ 55b4d521-0fe1-4642-8e75-e77936c602a2
const dx = dy = 2e-9

# ╔═╡ 003050be-1bc7-49a7-8dfa-cd4476c5c3fb
∇ = CenteredDifference{2}(2, dx);

# ╔═╡ a551d0e5-f3ca-4145-ac83-10a399ee0d19
md"""
Select the boundary conditions
"""

# ╔═╡ 0f869a1b-31e9-46a2-97aa-3873e2711241
@bind bc Select([PeriodicBC(), NeumannBC{2}(0.0)])

# ╔═╡ d626ca70-7384-46c7-980f-7f3e81f67615
md"""
Build some memory caches for our numerical integration
"""

# ╔═╡ 92880a73-00e1-4def-86bc-b450fd9d12d7
 E_ϕ, P_ϕ = LiquidCrystals.build_caches(∇, bc, ϕ₀);

# ╔═╡ 090cd661-d003-4492-9a4d-85612cae0e8f
 E_μ, P_μ = LiquidCrystals.build_caches(∇, bc, ϕ₀);

# ╔═╡ 00cd98fb-c2de-457a-a3d3-f8ed6924e9de
md"""
Define the discretized PDE as an ODE function
"""

# ╔═╡ 1996e1a9-f932-4908-b3f2-6f3f87c6f3d3

function cahn_hilliard(du, u, p, t)
    ∇, bc, E_ϕ, P_ϕ, E_μ, P_μ = p
    RT = R * T
    ...
    E_ϕ = LiquidCrystals.apply_BCs(∇, ϕ, bc, E_ϕ)
	# Apply Laplacian Operator on ϕ
    mul!(P_ϕ, ∇, E_ϕ)

    # Compute chemical potential
	φ = @view(P_ϕ[2:end-1, 2:end-1])
    μ = @. -K * φ + χ * (1 - 2 * ϕ) + RT * log(ϕ / (1 - ϕ))

    E_μ = LiquidCrystals.apply_BCs(∇, μ, bc, E_μ)	
    # Apply Laplacian Operator on μ
    mul!(P_μ, ∇, E_μ)
	
    @. dϕ =  Mc * @view(P_μ[2:end-1, 2:end-1])
	
	return du
end

# ╔═╡ 4f5e83db-3f66-4aef-911a-2c7dbf698d76
md"""
Set the time upper bound for evolving the dynamics and the number of snapshots we wish to save.
"""

# ╔═╡ 7196e0fe-2a25-4eb8-afd4-5cd126ff18d7
t_f = 55000

# ╔═╡ f6031e98-62ff-497c-b4ab-b6e7a0154756
Nt = 100

# ╔═╡ ba748588-553c-43cc-87af-19a4ed5616ed
md"""
Set the parameters and bundle them together with the ODE function, initial condition, time span in a `ODEProblem`.
"""

# ╔═╡ ba1480af-aa8c-4dfe-a8dc-7295b6511e9e
params = (∇, bc, E_ϕ, P_ϕ, E_μ, P_μ);

# ╔═╡ f41ce336-f6cb-4306-9ae6-bfbbde761ac9
prob = ODEProblem(cahn_hilliard, ϕ₀, (0.0, t_f), params);

# ╔═╡ 47a994f2-f06b-4b19-85ce-f60940ae41c1
md"""
Solve the ODE.
"""
...
md"""
Click on the Start button to watch the time evoltution of the solution.
"""

# ╔═╡ 7912d8d7-0240-46de-a638-8d718b4318d1
@bind nₜ PlutoUI.Clock(0.1, max_value = 100)

# ╔═╡ f3f0c088-fe85-46de-bf76-a0dcf72cef38
md"""
**Optional**

Save the solution as VTK files to visualize it  on Paraview
"""

# ╔═╡ 9dde16de-5593-46a5-b06d-5a4947af42bb
function writevtk(filename, space, ϕ)
	open(filename, "w") do file
		write(file, "# vtk DataFile Version 3.0 \nvtk output\nASCII\nDATASET UNSTRUCTURED_GRID \n")
		write(file, "POINTS $(length(ϕ)) float\n")
		write(file, space)
	

		write(file, "\n POINT_DATA $(length(ϕ)) \n")
		write(file, "SCALARS Phi float\nLOOKUP_TABLE default\n")
		
		write(file, join(ϕ, "\n"))
	end
end

# ╔═╡ f6aa40bc-9c68-413f-9f07-4b11d3bcbda9
...
# ╔═╡ 96503608-e07e-444f-9e2b-639e124d41ab
space = replace(
	join(Iterators.product(1:Nx, 1:Ny), " 0 \n"), r"[(,)]" => ""
) * " 0 \n";

# ╔═╡ e3cc6736-44a2-42e5-b786-d9ac6a292321
md"""
To save the data enable the cell below.
"""

# ╔═╡ a5a0305b-c59e-4422-ac48-6af37fec1466
# ╠═╡ disabled = true
#=╠═╡
paramovie(sol, space, Nt)
  ╠═╡ =#

# ╔═╡ 9707b3fb-48ef-4c38-96c1-aef71bb99044

begin

function visualize(ϕ)
...
# ╔═╡ 4e6786d2-c074-4bce-a78a-97510137aaec
# `visualize` defined at the end of the notebook
visualize(ϕ₀)

# ╔═╡ 85d9920e-8dec-488e-b234-26869543e851
visualize(sol[nₜ])
...
path = "/home/pzubieta/.julia/dev/LiquidCrystals"
...
git-tree-sha1 = "f9d84dcb46419e973872b32c051e5baad2d29de7"
